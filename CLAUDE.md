...
2. **Levels are pure data (JSON)** — adding a level requires zero code changes. *(test-enforced: schema validation; levels load with no code registration — guard: #38, planned; marking becomes true when the guard code merges)*
...
4. **Deterministic levels** — a level plays identically every run (seeded, fixed traffic patterns). *(test-enforced: replay/determinism guard test — guard: #39, planned; marking becomes true when the guard code merges)*
