...
1. **No ads, no tracking, no analytics, no network calls** — all player data stays on-device. *(test-enforced — guard: #17, merged: package blocklist + settings pin in the test suite, plus launcher-manifest permission removal verified against the final AAB in every release build)*
2. **Levels are pure data (JSON)** — adding a level requires zero code changes. *(test-enforced: schema validation; levels load with no code registration — guard: #38, merged: schema+referential validation over every shipped level, broken-fixture rejection tests)*
...
4. **Deterministic levels** — a level plays identically every run (seeded, fixed traffic patterns). *(test-enforced: replay/determinism guard test — guard: #39, merged: replay-hash + chunked-tick equivalence + no-RNG source guard)*
